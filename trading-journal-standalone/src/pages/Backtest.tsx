...
import { Play, Pause, SkipBack, SkipForward, Shuffle, RotateCcw, Upload, Loader2, Target } from 'lucide-react';
...
import { ChartDataset, Candle, BacktestTrade } from './data/types';
import ReplayChart from './ui/ReplayChart';
import UploadDatasetDialog from './ui/UploadDatasetDialog';
...
export default function Backtest() {
  ...
  // Playback loop - advances one candle every `speed` ms while playing.
  useEffect(() => {
    if (!playing || !candles) return;
    if (visibleCount >= candles.length) { setPlaying(false); return; }
    const t = setTimeout(() => setVisibleCount(v => Math.min(v + 1, candles.length)), SPEED_OPTIONS[speedIdx].ms);
    return () => clearTimeout(t);
  }, [playing, visibleCount, candles, speedIdx]);
  ...
  useEffect(() => {
    ...
    openTrades.forEach(t => {
      ...
      api.put(`/backtest?resource=trades&id=${t.id}`, {
        ...
        exit_price: exitPrice, result, rr, notes: t.notes,
  ...
  async function handleManualClose() {
    ...
    await api.put(`/backtest?resource=trades&id=${closingTrade.id}`, {
      ...
      exit_price: exitPrice, result, rr, notes: closingTrade.notes,
    });
    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <Button size="sm" onClick={() => setUploadOpen(true)}>
          <Upload className="w-3.5 h-3.5 mr-1" /> Upload Data
        </Button>
      </div>
      ...
      {datasets.length === 0 && (
        <Card><CardContent className="pt-6 pb-6 text-center text-sm text-muted-foreground">
          No candle data uploaded yet. Click <span className="font-medium text-foreground">Upload Data</span> to add a CSV export (TradingView, MT4/MT5, Dukascopy, or HistData.com all work).
        </CardContent></Card>
      )}
        ...
        <div className="flex flex-col gap-4">
          <Card>
            <CardContent className="pt-4 pb-4 flex flex-col gap-3">
              ...
              <ReplayChart candles={candles} visibleCount={visibleCount} trades={trades} />
            </CardContent>
          </Card>

          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4">
            <LogTradeForm lastClose={candles[Math.max(0, visibleCount - 1)].close} onSubmit={handleLogTrade} />
            <BacktestLog trades={trades} onCloseTrade={(t) => { setClosingTrade(t); setClosePrice(''); }} onDeleteTrade={handleDeleteTrade} />
          </div>
        </div>
      )}

      <UploadDatasetDialog open={uploadOpen} onClose={() => setUploadOpen(false)} onUploaded={refetchDatasets} />
