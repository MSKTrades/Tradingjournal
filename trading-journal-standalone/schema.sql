...
-- accounts/strategies/checklists/tags/tag_groups/daily_routine_notes (see the
-- multi-tenancy migration further down this file) - this stopped being a
-- single-user-per-login product once real signups were on the table.
-- password_hash is a bcrypt hash, never a plaintext password - and is NULL
-- for accounts created via Google (OAuth-only accounts never get a local
-- password unless the person later sets one).
-- ============================================================================
...
-- ============================================================================
-- MIGRATION: real multi-tenancy. Every table below (accounts, strategies,
-- checklists, tags, tag_groups, daily_routine_notes) was previously global —
-- no user_id column anywhere, and no API handler filtered by one either. That
-- was fine when this was a single person's own journal with a login screen
-- bolted on just to keep the app off the open internet, but it means every
-- signed-up user currently reads and writes the exact same shared data.
-- This section adds real ownership. trades, custom_columns, checklist_items,
-- and tag_group_options don't get their own user_id — they're owned
-- transitively through account_id / checklist_id / group_id, which is
-- checked in the API layer (api/_auth.js's ownsAccount / ownsChecklist /
-- ownsTagGroup) rather than duplicated as a column here.
--
-- Backfill strategy: every existing row (all created before any of this
-- existed) is assigned to the OLDEST registered user — on today's production
-- database that's you, so this reproduces exactly what you have today under
-- your account. If more than one real user has already signed up by the time
-- this runs, review the backfill below before applying it — it does not
-- try to guess which rows belong to whom.
-- ============================================================================
...
ALTER TABLE tag_groups           ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE daily_routine_notes  ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE CASCADE;

DO $$
DECLARE
  default_user_id INTEGER;
BEGIN
  SELECT id INTO default_user_id FROM users ORDER BY created_at ASC, id ASC LIMIT 1;
  IF default_user_id IS NOT NULL THEN
    UPDATE accounts            SET user_id = default_user_id WHERE user_id IS NULL;
    UPDATE strategies          SET user_id = default_user_id WHERE user_id IS NULL;
    UPDATE checklists          SET user_id = default_user_id WHERE user_id IS NULL;
    UPDATE tags                SET user_id = default_user_id WHERE user_id IS NULL;
    UPDATE tag_groups          SET user_id = default_user_id WHERE user_id IS NULL;
    UPDATE daily_routine_notes SET user_id = default_user_id WHERE user_id IS NULL;
  END IF;
END $$;

-- Lock user_id down as NOT NULL once every existing row has one — guarded so
-- this is a no-op (not an error) on a fresh install with zero users and zero
-- rows yet; every row created from here on always supplies user_id at INSERT
-- time (see api/columns.ts signup / api/accounts.ts createAccount / etc.).
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM accounts WHERE user_id IS NULL) THEN
    ALTER TABLE accounts ALTER COLUMN user_id SET NOT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM strategies WHERE user_id IS NULL) THEN
    ALTER TABLE strategies ALTER COLUMN user_id SET NOT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM checklists WHERE user_id IS NULL) THEN
    ALTER TABLE checklists ALTER COLUMN user_id SET NOT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM tags WHERE user_id IS NULL) THEN
    ALTER TABLE tags ALTER COLUMN user_id SET NOT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM tag_groups WHERE user_id IS NULL) THEN
    ALTER TABLE tag_groups ALTER COLUMN user_id SET NOT NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM daily_routine_notes WHERE user_id IS NULL) THEN
    ALTER TABLE daily_routine_notes ALTER COLUMN user_id SET NOT NULL;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_daily_routine_notes_user  ON daily_routine_notes (user_id);

-- tags.name and tag_groups.name were globally UNIQUE (one "A+ Setup" tag for
-- the whole app); now that each user has their own set, uniqueness needs to
-- be per-user instead — otherwise the second person to ever create a tag
-- named "A+ Setup" would get an error. daily_routine_notes.note_date was
-- UNIQUE globally (one note per calendar date, for everyone); same fix,
-- scoped to (user_id, note_date) so each person has their own daily note per
-- date.
ALTER TABLE tags DROP CONSTRAINT IF EXISTS tags_name_key;
CREATE UNIQUE INDEX IF NOT EXISTS tags_user_name_idx ON tags (user_id, lower(name));

ALTER TABLE tag_groups DROP CONSTRAINT IF EXISTS tag_groups_name_key;
CREATE UNIQUE INDEX IF NOT EXISTS tag_groups_user_name_idx ON tag_groups (user_id, lower(name));
